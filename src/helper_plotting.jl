...
function plot_diss_fit!(plt,M,rates,data;cmap=nothing,label=nothing)

    sols = []
    for i in 1:length(M.cons)
        # Setup the ODE problem, then solve
        # sol = solve_model(M,rates,M.dens[i],M.cons[i])
        sol = run_model(M,rates,M.dens[i],M.cons[i],data.t[i])
    ...
    p_ = plot_dissociations!(plt,data,colors)
    return sols, p_
end
...
function show_diss_fit!(plt,M,rates,data;cmap=nothing,label=nothing)

    sols, p_ = plot_diss_fit!(plt,M,rates,data;cmap,label)
    return sols, p_
end
# function Im2Re(S)
#     Σ = Complex.(S)
